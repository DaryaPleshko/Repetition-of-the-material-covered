...
const sumOfDigits = (num1, num2) => {
    ...
    if (sum1 > sum2)
        return `${num1} имеет большую сумму цифр`;
    else if (sum2 > sum1)
        return `${num2} имеет большую сумму цифр`;
    else
        return "Оба числа имеют одинаковую сумму цифр";
};
...
const includeVal = (str1, str2) => {
    if (str1.length !== str2.length)
        return "Строки не содержат одинаковые символы";
...
const isEven = (num1, num2) => ((num1 + num2) % 2 === 0) ? 'Сумма четная' : 'Сумма нечетная';
...
const isFirstVowel = (str) => (/^[aeiou]/i.test(str)) ? "Строка начинается с гласной" : "Строка не начинается с гласной";
...
const canSplitString = (str) => {
    ...
    for (let i = 1; i < str.length; i++) {
        ...
        if (leftUnique.size === rightUnique.size)
            return `Можно (${leftPart}|${rightPart})`;
    }
    return "Нельзя";
};
...
const checkConsecutiveProduct = (num) => {
...
const isPalindrom = (str) => (str === str.split('').reverse().join('')) ? 'Можно' : 'Нельзя';
